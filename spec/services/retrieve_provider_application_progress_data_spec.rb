...
RSpec.describe RetrieveProviderApplicationProgressData do
  describe '#execute' do
    AVAILABLE_STATUSES = %i[awaiting_provider_decision interviewing offer pending_conditions recruited]
    let(:provider) { create(:provider) }
    let(:course) { create(:course, provider: provider) }
    let(:course_option) { create(:course_option, course: course) }
    let(:accredited_course) { create(:course, accredited_provider: provider) }
    ...
    it 'generates the correct count' do
      expect(described_class.new(provider: provider).exec.map(&:count).inject(:+)).to eq(26)
    end

    it 'generates the correct SQL' do
      puts described_class.new(provider: provider).exec.to_sql
    end

    it 'can group the courses by id' do
      courses = described_class.new(provider: provider).exec.group_by(&:id)
