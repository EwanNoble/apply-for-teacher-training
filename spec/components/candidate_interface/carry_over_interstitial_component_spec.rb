...
RSpec.describe CandidateInterface::CarryOverInterstitialComponent do
  ...
  context 'after the new recruitment cycle begins' do
    ...
    it 'renders the correct cycle names' do
      application_form = build(:completed_application_form, recruitment_cycle_year: RecruitmentCycle.previous_year)
      result = render_inline(described_class.new(application_form: application_form))

      expect(result.text).to include('You started an application for courses starting in the 2020 to 2021 academic year, which have now closed.')
      expect(result.text).to include('Continue your application to apply for courses starting in the 2021 to 2022 academic year instead.')
    end
  end
