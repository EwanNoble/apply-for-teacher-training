...
RSpec.describe ProviderRelationshipPermissions do
  ...
  describe "#governs_open_course?" do
    let(:training_provider) { create(:provider) }
    let(:ratifying_provider) { create(:provider) }
    let!(:relationship) do
      create(
    ...
    context "there are no courses" do
      it "returns false" do
        expect(relationship.governs_open_course?).to be(false)
      end
    end
    ...
    context "there is an open course" do
      ...
      it "returns false" do
        expect(relationship.governs_open_course?).to be(true)
      end
    end
  end

  describe ".govern_open_course" do
    def create_relationship_with_course(traits: [])
      relationship = create (:provider_relationship_permissions)
      create(:course, *traits, provider: relationship.training_provider, accredited_provider: relationship.ratifying_provider)
      relationship
    end
    ...
    let!(:open_course_relationship) { create_relationship_with_course(traits: [:open_on_apply]) }
    let!(:open_course_in_previous_cycle_relationship) { create_relationship_with_course(traits: [:open_on_apply, :previous_year]) }

    it 'only returns the open_course_relationship' do
      expect(described_class.governs_open_course).to eq([open_course_relationship])
    end
  end
end
